...
var fontSize = core_1.system({ fontSize: true });
var color = core_1.system({ color: true });
test('variant returns style objects from theme', function () {
    var buttons = variant_1.variant({ key: 'buttons' });
    var a = buttons({
        theme: {
            buttons: {
                primary: {
...
test('variant prop can be customized', function () {
    var buttons = variant_1.variant({ key: 'buttons', prop: 'type' });
    var a = buttons({
        theme: {
            buttons: {
                primary: {
...
test('variant can be composed', function () {
    var system = core_1.compose(variant_1.variant({ key: 'typography' }), fontSize, color);
    var result = system({
        theme: {
            typography: {
                primary: {
...
describe('component variant', function () {
    test('returns a variant defined inline', function () {
        ...
        var primary = comp({ variant: 'primary' });
        var secondary = comp({ variant: 'secondary' });
    ...
    test('can be composed with other style props', function () {
        var parser = core_1.compose(variant_1.variant({
            variants: {
                tomato: {
                    color: 'tomato',
                    fontSize: 20,
